...
public class Facebook {
     ...
     * @param applicationId The Facebook application identifier e.g.
     *            "350685531728"
     * @param permissions A list of permissions required for this application:
     *            e.g. "read_stream", "publish_stream", "offline_access", etc.
     *            see
     *            http://developers.facebook.com/docs/authentication/permissions
     *            This parameter should not be null -- if you do not require any
     *            permissions, then pass in an empty String array.
     * @param activityCode Single sign-on requires an activity result to be
     *            called back to the client application -- if you are waiting on
     *            other activities to return data, pass a custom activity code
     *            here to avoid collisions. If you would like to force the use
     *            of legacy dialog-based authorization, pass FORCE_DIALOG_AUTH
     *            for this parameter. Otherwise just omit this parameter and
     *            Facebook will use a suitable default. See
     ...
     * 
     * @param activity The Android Activity that will parent the ProxyAuth
     *            Activity.
     * @param applicationId The Facebook application identifier.
     * @param permissions A list of permissions required for this application.
     *            If you do not require any permissions, pass an empty String
     *            array.
     * @param activityCode Activity code to uniquely identify the result Intent
     *            in the callback.
     */
     ...
     *            application when the refresh request has completed or failed
     *            (can be null). In case of a success a new token can be found
     *            inside the result Bundle under Facebook.ACCESS_TOKEN key.
     ...
     *            it should be the same context in which the login occurred in
     ...
     *            "me", which will fetch http://graph.facebook.com/me
     * @throws IOException
     * @throws MalformedURLException
     * @return JSON string representation of the response
     */
     ...
     * 
     * @param expiresIn - duration in seconds (or 0 if the session doesn't
     *            expire)
     */
     ...
     * 
     * @param context
     * @return returns false initially, but will eventually transition to true
     *         if success is reached by the async task.
     */
     ...
     * @throws Exception
     */
    private static void publishInstall(final Facebook fb, final String applicationId,
        ...
        if (lastPing == 0 && attributionId != null) {
            ...
            JSONObject supportResponse = Util.parseJson(fb.request(applicationId,
                    supportsAttributionParams));
            ...
            if (!(doesSupportAttribution instanceof Boolean)) {
                throw new JSONException(String.format(
                        "%s contains %s instead of a Boolean", SUPPORTS_ATTRIBUTION,
        ...
        synchronized (this) {
            if (mAutoPublishAsyncTask == null && (ignoreSetting || getShouldAutoPublishInstall())) {
                ...
                if (applicationId != null) {
                    asyncTask = mAutoPublishAsyncTask = new AutoPublishAsyncTask(applicationId,
                            context);
                }
            }
        }
